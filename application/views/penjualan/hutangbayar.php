...
<div class="my-3 p-3 bg-white rounded shadow-sm">
	<h6 class="border-bottom border-gray pb-2 mb-0">Laporan Data Pembayaran Piutang</h6>
...
if(isset($_POST['test'])){
	if($_POST['sp']<>''){
			$q=$this->db->query("select * from pembayaran where supplier='$_POST[sp]'  and gudang='$toko' and jenis='M' order by id desc");
	}elseif($_POST['te']<>''){
			$q=$this->db->query("select * from pembayaran where nobil='$_POST[te]'  and gudang='$toko'  and jenis='M'");
	}else{
			$q=$this->db->query("select * from pembayaran
			where tglbayar>='$_POST[tgla]' and tglbayar<='$_POST[tglb]'  and gudang='$toko'  and jenis='M' order by id desc");
	}
}else{
		$q=$this->db->query("select * from pembayaran where tglbayar=date(now())  and gudang='$toko'  and jenis='M' order by id desc");
}	$byr=0;
