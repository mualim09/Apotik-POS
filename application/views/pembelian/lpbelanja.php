...
if(isset($_POST['test'])){
if($_POST['sp']<>''){
		$q=$this->db->query("select keluar_masuk.*,sum(jml) as jumlah,sum(jml*replace(modal,',','')) as nota,gudang.gudang from keluar_masuk  
		inner join gudang on gudang.id=keluar_masuk.gudang where keluar_masuk.idtoko='$idtoko' and keluar_masuk.supplier='$_POST[sp]'  and  keluar_masuk.ket='M' group by nofaktur  order by keluar_masuk.id desc");
}elseif($_POST['te']<>''){
		$q=$this->db->query("select keluar_masuk.*,sum(jml) as jumlah,sum(jml*replace(modal,',','')) as nota,gudang.gudang from keluar_masuk  
		inner join gudang on gudang.id=keluar_masuk.gudang where keluar_masuk.idtoko='$idtoko' and keluar_masuk.nofaktur='$_POST[sp]' and  keluar_masuk.ket='M' group by nofaktur  order by keluar_masuk.id desc");
}else{
		$q=$this->db->query("select keluar_masuk.*,sum(jml) as jumlah,sum(jml*replace(modal,',','')) as nota,gudang.gudang from keluar_masuk  
		inner join gudang on gudang.id=keluar_masuk.gudang
		where  keluar_masuk.idtoko='$idtoko' and keluar_masuk.tgl>='$_POST[tgla]' and keluar_masuk.tgl<='$_POST[tglb]' and keluar_masuk.ket='M' group by nofaktur  order by keluar_masuk.id desc");
}
}else{
		$q=$this->db->query("select keluar_masuk.*,sum(jml) as jumlah,sum(jml*replace(modal,',','')) as nota,gudang.gudang from keluar_masuk  
		inner join gudang on gudang.id=keluar_masuk.gudang where     keluar_masuk.tgl=date(now()) and   keluar_masuk.idtoko='$idtoko' and keluar_masuk.ket='M' group by nofaktur  order by keluar_masuk.id desc");
