...
<div class="my-3 p-3 bg-white rounded shadow-sm">
	<h6 class="border-bottom border-gray pb-2 mb-0">Laporan Data Return Pembelian</h6>
...
if(isset($_POST['test'])){
if($_POST['sp']<>''){
		$q=$this->db->query("select keluar_masuk.*,gudang.gudang as nmgudang from keluar_masuk  
		inner join gudang on gudang.id=keluar_masuk.gudang where keluar_masuk.idtoko='$idtoko' and  keluar_masuk.supplier='$_POST[sp]'  and  keluar_masuk.ket='RB'  group by nofaktur  order by keluar_masuk.id desc");
}elseif($_POST['te']<>''){
		$q=$this->db->query("select keluar_masuk.*,gudang.gudang as nmgudang from keluar_masuk  
		inner join gudang on gudang.id=keluar_masuk.gudang where keluar_masuk.idtoko='$idtoko' and  keluar_masuk.nofaktur='$_POST[sp]' and  keluar_masuk.ket='RB' group by nofaktur  order by keluar_masuk.id desc");
}else{
		$q=$this->db->query("select keluar_masuk.*,gudang.gudang as nmgudang from keluar_masuk  
		inner join gudang on gudang.id=keluar_masuk.gudang
		where keluar_masuk.idtoko='$idtoko' and  keluar_masuk.tgl>='$_POST[tgla]' and keluar_masuk.tgl<='$_POST[tglb]' and keluar_masuk.ket='RB' group by nofaktur  order by keluar_masuk.id desc");
}
}else{
		$q=$this->db->query("select keluar_masuk.*,gudang.gudang as nmgudang from keluar_masuk  
		inner join gudang on gudang.id=keluar_masuk.gudang where keluar_masuk.idtoko='$idtoko' and   keluar_masuk.ket='RB' group by nofaktur  order by keluar_masuk.id desc limit 10");
		...
		<tr>
			...
			<?php echo $row->nmgudang; ?></td>
			<td><?php echo $row->nofaktur; ?><br /><?php echo $row->supplier; ?></td>
			<td>
			<?php echo $row->kode; ?> <?php echo $row->nama; ?>
			<br />			<b class='badge badge-info'><?php echo $row->stbayar; ?></b>
		...
		$(".xhapusnota").click(function(e){
						...
						$.ajax({
							...
							success: function(dt){
								...
								window.location="pembelian/xreturn"
							},
							error: function(dt){
